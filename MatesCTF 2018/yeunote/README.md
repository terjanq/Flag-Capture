...
We can create a note containg `title` and `description` with two options avaible: `public` and `private`. If we choose the `private` option then only people added to `share list` can see the note. Additionaly, each note can be protected with the `password`. 

On the main page we see the the titles of random *public* notes and their `id`. 
...
Where `bdf0ae82-2284-4b7f-a0b3-26e6d291ff1f` is a value from the  `csrf` cookie. We can change it to `')|alert('xss` which results with sucesfully attached `xss injection`. It is also the only place on the site where I found the mentioned vulnerability but it seems to be not exploitable. 
...
So what we want to do is to craft a `csrf page` wich will allow us to do malicious things using admin's account. But every form is protected with `crsf token` and we have to somehow steal admin's `crsf` cookie. Have we? 

Earlier posted script adding `crsf token` is hosted on `https://yeunote.ctf.yeuchimse.com/csrf.js` so we can just attach this script on our page. But if we try that, we get the `no permissions` error or something like that. Quick inspection showed that the reason of this is the `referrer header` set to `https://yoursite`. 
...
After previous step we are now able to visit `https://yeunote.ctf.yeuchimse.com/note/view/3` but it results with `password` prompt. So our task now is to get the password somehow.
...
The third one is using backend app. In order to successfully post the tweet we have to authorize the app before clicking the button, but only once. Clicking on the button redirects us to `https://yeunote.ctf.yeuchimse.com/note/twitter/<note id>` which if we have the note with the `<note id>` in our inventory redirects us to `https://api.twitter.com/oauth/authenticate?oauth_token=<oauth token>`. If we already authorized the app the it will redirect us to `https://yeunote.ctf.yeuchimse.com/note/twitter?oauth_token=<oauth token>&oauth_verifier=<oauth verifier>` and if everything went allright we will see the post containg an URL with the password contained on our twitter account. 
...
My attempt was to login the admin to my dummy twitter account. I could achieve that using similar method as for the previous step but with a little help of backed script generating the `authenticity_token`. But it didn't work because twitter is not accepting any referrer different from `https://twitter.com` including `no referrer`. 

I was also trying to generate `oath token` for flag note which would allow me to terminate the redirection from `twitter` to `yeunote` and send the generated url to the admin. But this try results with `Note not found` error and I couldn't find any way to bypass it.
